...
import type { ComunicadoInput, CategoriaComunicado } from "@/lib/types/noticias";
...
// ── COMPONENTE ─────────────────────────────────────────────────────────
export default function CrearComunicadoPage() {
  const router = useRouter();
  const [form, setForm]                 = useState<ComunicadoInput>({ ...EMPTY_FORM, fechaPublicacion: new Date().toISOString().slice(0, 10) });
  ...
  async function handleAdjuntoUpload(e: ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      router.push("/dashboard/admin/comunicados");
    } catch {
      ...
      <div className="px-6 md:px-10 lg:px-16 pt-10 pb-16">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="rounded-xl p-6" style={{ background: "var(--blanco)", border: "1px solid var(--gris-borde)" }}>
            <h2 className="text-base font-bold mb-5" style={{ color: "var(--texto-primario)" }}>
              Nuevo comunicado
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                {imagenModo === "url" ? (
                  <>
                    ...
                    {form.imagenUrl && (
                      <div className="relative mt-2">
                        <img src={form.imagenUrl} alt="preview" className="rounded-lg w-full object-cover" style={{ height: "140px" }} onError={(e) => (e.currentTarget.style.display = "none")} />
                        <button type="button" onClick={() => setForm((f) => ({ ...f, imagenUrl: null }))} className="absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center text-xs" style={{ background: "rgba(0,0,0,0.55)", color: "#fff" }}>×</button>
                      </div>
                    )}
                  </>
                ) : (
                  <>
                    ...
                    {form.imagenUrl && (
                      <div className="relative mt-2">
                        <img src={form.imagenUrl} alt="preview" className="rounded-lg w-full object-cover" style={{ height: "140px" }} />
                        <span className="absolute top-2 left-2 text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(22,163,74,0.85)", color: "#fff" }}><i className="bi bi-check-lg" style={{ fontSize: "12px", marginRight: "2px" }} /> Subida correctamente</span>
                        <button type="button" onClick={() => setForm((f) => ({ ...f, imagenUrl: null }))} className="absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center text-xs" style={{ background: "rgba(0,0,0,0.55)", color: "#fff" }}>×</button>
                      </div>
                    )}
                  </>
              ...
              <FormField label="Categoría">
                <div className="flex gap-2 flex-wrap">
                  {CATEGORIAS.map((cat) => {
                    ...
                    const col = CATEGORIA_COLORES[cat];
                    return (
              ...
              <FormField label="Opciones">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <input type="checkbox" checked={form.destacado} onChange={(e) => setForm({ ...form, destacado: e.target.checked })} className="w-4 h-4 rounded" style={{ accentColor: "var(--azul-egm)" }} />
              ...
              <FormField label="Fecha de publicación">
                <input type="date" value={form.fechaPublicacion ?? ""} onChange={(e) => setForm({ ...form, fechaPublicacion: e.target.value || null })}
                  className="w-full rounded-lg px-3 py-2 text-sm focus:outline-none"
                  style={{ border: "1px solid var(--gris-borde)", background: "var(--blanco)", color: "var(--texto-primario)" }}
                />
              </FormField>
              ...
              <FormField label="Fecha de expiración (opcional)">
                <input type="date" value={form.fechaExpiracion ?? ""} onChange={(e) => setForm({ ...form, fechaExpiracion: e.target.value || null })}
                  className="w-full rounded-lg px-3 py-2 text-sm focus:outline-none"
                  style={{ border: "1px solid var(--gris-borde)", background: "var(--blanco)", color: "var(--texto-primario)" }}
                />
              ...
              <div className="md:col-span-2">
                ...
                <input type="url" value={form.videoUrl ?? ""} onChange={(e) => setForm({ ...form, videoUrl: e.target.value || null })}
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex flex-col gap-2">
                  <input type="url" value={form.enlaceUrl ?? ""} onChange={(e) => setForm({ ...form, enlaceUrl: e.target.value || null })}
                    placeholder="https://..." className="w-full rounded-lg px-3 py-2 text-sm focus:outline-none"
                    style={{ border: "1px solid var(--gris-borde)", background: "var(--blanco)", color: "var(--texto-primario)" }}
                  />
                  {form.enlaceUrl && (
                    <input type="text" value={form.enlaceTexto ?? ""} onChange={(e) => setForm({ ...form, enlaceTexto: e.target.value || null })}
              ...
              <div className="md:col-span-2">
                <label className="flex items-center gap-2.5 cursor-pointer select-none">
                  <input type="checkbox" checked={form.estado === "borrador"} onChange={(e) => setForm({ ...form, estado: e.target.checked ? "borrador" : "publicado" })}
            ...
            <div className="flex gap-2 justify-end pt-4 mt-4" style={{ borderTop: "1px solid var(--gris-superficie)" }}>
              <button onClick={() => router.push("/dashboard/admin/comunicados")} className="text-sm px-4 py-2 rounded-lg" style={{ color: "var(--texto-secundario)" }}>
                Cancelar
              </button>
              ...
              >
                {submitting ? "Guardando..." : form.estado === "borrador" ? "Guardar borrador" : "Publicar comunicado"}
              </button>
            </div>
          </div>
        </div>
