...
import { DEPARTAMENTOS, EMPTY_EMPLEADO, ROL_EMPLEADO_ID } from "@/lib/constants/admin";
...
// ── Component ────────────────────────────────────────────────────────────────
export function EmpleadosAdminTab({ empleados, cargandoEmpleados, empresaId, onToast }: Props) {
    ...
    try {
      const res = await apiFetch(`${API_URL}/users/${empleadoSeleccionado.usuarioId}`, {
        ...
        body: JSON.stringify({
          ...
          email: editEmpleadoForm.email.trim() || undefined,
      ...
      queryClient.invalidateQueries({ queryKey: QK.empleados(empresaId) });
      setEditandoEmpleado(false);
      onToast("Datos del empleado guardados correctamente");
    } catch (err) {
      onToast(err instanceof Error ? err.message : "Error al guardar los datos del empleado", "error");
    } finally {
  ...
  const ejecutarResetPassword = async (nuevaPassword: string) => {
    ...
    try {
      ...
      onToast(`Contraseña de ${resetPass.nombre} actualizada correctamente`);
      setResetPass(null);
    } catch (err) {
      onToast(err instanceof Error ? err.message : "Error al resetear la contraseña", "error");
    } finally {
  ...
  const exportarEmpleadosExcel = async () => {
    ...
    try {
      ...
      onToast(`Excel exportado con ${empleados.length} empleados`);
    } catch {
      onToast("Error al exportar el archivo", "error");
    } finally {
      setExportando(false);
    }
  };
  ...
  const importarEmpleados = async (file: File) => {
    ...
    queryClient.invalidateQueries({ queryKey: QK.empleados(empresaId) });
    if (errores.length === 0) {
      onToast(`${ok} empleado${ok !== 1 ? "s" : ""} importado${ok !== 1 ? "s" : ""} correctamente`);
    } else if (ok > 0) {
      onToast(`${ok} importado${ok !== 1 ? "s" : ""}, ${errores.length} con error`, "error");
    } else {
      onToast("Error al importar el archivo", "error");
    }
  };
  ...
  return (
    <div className="relative">
      ...
      {/* ── Modal confirmación toggle empleado ── */}
      <ModalConfirm
